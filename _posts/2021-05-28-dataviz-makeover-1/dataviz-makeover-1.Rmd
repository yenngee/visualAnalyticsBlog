...
#### Step 11 
Now to add annotations. 
we want the annotation to point to several arrows. Unfortunately Tableau has yet to have this function. Hence, to achieve this the trick is to create multiple annotations from the different points of concern, thereafter overlapping the boxes so that the lines point towards the same annotation. Then we just need to format the boxes and arrow such that they are consistent. 
We use dotted line with an open arrow and a box that has no shading. 

![](img/step-C11_format.jpg)
