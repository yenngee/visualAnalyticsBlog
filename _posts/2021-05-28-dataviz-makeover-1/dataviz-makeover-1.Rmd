...
```{r fig.cap="Original Visualization", out.extra="class=external"}
...
As we are looking at Singapore's top 6 merchandise trading partners and not just each countries' export and import, it was decided to use stacked area chart to visualize the merchandise trade (sum of import and export) which shows the part to whole relationship well. Hence, we are also able to see the proportion of import and export relative to the sum of them like how Hong Kong has a very low import but actually has higher overall merchandise trade than that of Japan. This overcomes the issues raised in **_C6_** and [**_A5_**](#by-aesthetics).
...
![](img/final.jpg)
The visualization can also be access on [Tableau Public](https://public.tableau.com/app/profile/yen.ngee/viz/SingaporeTop6TradingPartners/Sheet1)
...
```{r fig.cap="screenshot of top few rows of T1", out.extra="class=external"}
knitr::include_graphics("img/step-A2_T1_top.jpg")
```


```{r fig.cap="screenshot of bottom few rows of T1", out.extra="class=external"}
...
![](img/step-B3_filter.jpg)


#### **Step 3b: Clean T1 - Split column to get country** 

If we look under variables, we know that it is in the format of "white space" + "country name" + "(Thousand Dollars)".
We want to only extract the country name, keeping in mind the values are in thousands. Hence, if we click the 3 dots on the column, we can do an automatic split which will extract country name. The original column is then removed as we do not need it and the new column name is changed to Country.

![](img/step-B3_split.jpg)

The data after this step should look like this: 
![](img/step-B3_after_clean.jpg)
...
![](img/step-B5_after_pivot.jpg)

Note: a manual check confirms that the 6 countries that we are concerned with is part of this data set.


#### **Step 6: Clean **
At this point, it is easy to convert the Import(in thousands) and Export(in thousands) to the values multiply by 1000.
Clicking the 3 dots, we select Create Calculated Field, then Custom Calculation. 

![](img/step-B6_calculated_field.jpg)
...
![](img/step-B6_calculated_field2.jpg)
...
Before we begin, it is always good to have cursory check on the data type. We see that Country, Date and Trade Type are dimensions while Amount is in Measure.
...
![](img/step-C1_drag_and_drop.jpg)

#### **Step 2:** 
We can see from the screenshot above there are too many countries. Hence, we need to filter the data by Country.
Drag and drop Country onto the Filter card next to the graph. A window will pop-up (see screenshot below). From there we will manually select the top 6 countries: United States, Malaysia, Hong Kong, Japan, Mainland China and Taiwan. 

![](img/step-C2_filter_country.jpg)

Results will look like this: 
![](img/step-C2_filter_country_after.jpg)
...
![](img/step-C5_change_to_quarter.jpg)

This is the result: 
![](img/step-C5_result.jpg)
...
The results will be something like this: 
![](img/step-C10_results.jpg)

#### **Step 11 **
Now to add annotations and reference lines 
Right-click on the white space to select annotations, and then Area, will pop-up a window for editing the annotation.
For this project, we select `Font` = "Tableau Book" and `Font Size` = 9. We place the annotations in areas where the content of the annotations makes the most sense, taking into consideration the white spaces in the visualization. 


![](img/step-C11_annotate.jpg)
